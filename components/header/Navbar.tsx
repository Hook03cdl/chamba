...
export default async function Navbar() {
	...
	return (
		<header className="sticky z-40 top-0 bg-humo">
			<div className="flex justify-between items-center p-3 lg:px-10 shadow-md">
				...
				<div className="flex gap-3 items-center">
					{session && (
						<>
							<Popover
								fallback={<MessageCircle size={28} className="text-gray-600" />}
							>
								<div className="min-w-80">
									{chats.message ? (
										...
										<Each
											of={chats}
											render={(c: ChatProps) => (
												<PopLink href={`/chat/${c.uuid}?user=${c.worker.name}`}>
